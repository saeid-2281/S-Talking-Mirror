...
class MainWindow(QMainWindow):
    def __init__(self, context: ApplicationContext):
        ...
        self.project_path=None; self.generation_started_at=None; self.run_logs=[]; self.report_dialogs=[]; self.palette=None; self.actions_by_name={}; self.job_pronunciation_overrides={}
    ...
    def build(self):
        ...
        self.table=QTableWidget(0,8); self.table.setHorizontalHeaderLabels(['#','Filename','Text','Chars','Status','Time','Retry','Provider']); self.table.setSelectionBehavior(QTableWidget.SelectRows); self.table.setContextMenuPolicy(Qt.CustomContextMenu); self.table.horizontalHeader().setSectionResizeMode(2,QHeaderView.Stretch); self.table.itemSelectionChanged.connect(self.preview); self.table.itemSelectionChanged.connect(self.update_queue_actions); self.table.customContextMenuRequested.connect(self.queue_context_menu); self.table.cellDoubleClicked.connect(lambda *_: self.play_selected_output()); ml.addWidget(self.table); split.addWidget(mid)
    ...
    def refresh_monitor_queue(self):
        self.monitor_service.refresh_queue(self.generation_controller.jobs,provider=self.provider.currentText(),output_dir=Path(self.out.text() or self.project_controller.default_output_path),settings=self.settings())
    def restore_layout_state(self):
        settings=QSettings('S Talking','S Talking'); state=settings.value('main_window/state'); visible=settings.value('main_window/monitor_visible',True,type=bool); width=settings.value('main_window/monitor_width',MONITOR_DEFAULT_WIDTH,type=int)
        if state:
            try:
    ...
    def save_layout_state(self):
        settings=QSettings('S Talking','S Talking'); settings.setValue('main_window/state',self.saveState())
        if hasattr(self,'monitor_dock'):
    ...
    def render_queue(self):
        ...
        for r,j in enumerate(jobs):
            values=[j.row_number,j.filename,j.text,f'{j.character_count:,}',j.status.value,f'{j.duration_seconds:.2f}s' if j.duration_seconds else '—',j.retry_count,self.provider.currentText()]
    ...
    def preview(self):
        ...
        if rows:
            ...
            j=selected[0]; out=self.generation_controller.output_path_for(j,Path(self.out.text() or self.project_controller.default_output_path),self.settings()); color=COLORS.get(j.status.value,'#94A3B8'); self.pname.setText(j.filename); self.pstatus.setText(j.status.value.title()); self.pstatus.setStyleSheet(f'background:{color};color:white;border-radius:6px;padding:3px 7px;font-weight:700;'); self.pmeta.setText(f'Row {j.row_number} • {len(j.text):,} characters • {self.provider.currentText()} • Voice {self.voice.text() or "—"}'); self.poutput.setText(f'Output: {elide_middle(str(out),54)}'); self.poutput.setToolTip(str(out)); self.ptext.setPlainText(j.text)
        else:
            self.pname.setText('No row selected'); self.pstatus.clear(); self.pmeta.setText('Load a CSV and select a row to inspect it.'); self.poutput.clear(); self.ptext.setPlainText('The selected row preview will appear here after a CSV is loaded.')
    def dashboard(self):
        self.refresh_quota_snapshot()
