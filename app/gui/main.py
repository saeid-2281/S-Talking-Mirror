...
class MainWindow(QMainWindow):
    def __init__(self, context: ApplicationContext):
        ...
        self.financial_audit_service=context.financial_audit_service
        self.provider_governance_service=context.provider_governance_service
        self.operations_command_center_service=context.operations_command_center_service
        self.evidence_refresh_service=context.evidence_refresh_service
    ...
    def open_provider_cost_quota_limits(self):
        ...
        return dialog
    def open_danish_provider_benchmark(self):
        dialog=DanishProviderBenchmarkDialog(
            self.context.danish_provider_benchmark_service,
            open_path=self.context.desktop_service.open_path,
            parent=self,
        )
        dialog.openCatalogRequested.connect(self.open_unified_voice_model_catalog)
        dialog.show()
        self.danish_provider_benchmark_dialog=dialog
        return dialog
    ...
    def handle_notification_action(self,payload):
        ...
        handlers={
            ...
            'operational-readiness-certification':self.open_operational_readiness,
    ...
    def open_financial_audit(self):
        dialog=FinancialAuditDialog(
            self.financial_audit_service,
            self,
            open_path=self.open_path,
        )
    ...
    def open_release_lifecycle_validation(self):
        ...
        dialog.openRequested.connect(self._open_operations_tool)
        return self._show_report_dialog(dialog,category='release-lifecycle-validation')
    def open_operational_persistence(self):
        ...
        return self._show_report_dialog(dialog,category='operational-persistence')
    def open_evidence_refresh(self):
        project=self.project_controller.current_project
        dialog=EvidenceRefreshDialog(
            self.evidence_refresh_service,
            self,
            project_id=project.project_id if project else None,
            open_path=self.open_path,
        )
        return self._show_report_dialog(dialog,category='evidence-refresh')
    def open_operational_readiness(self):
        project=self.project_controller.current_project
        dialog=OperationalReadinessDialog(
            self.operational_readiness_service,
            self,
            project_id=project.project_id if project else None,
            open_path=self.open_path,
        )
        return self._show_report_dialog(dialog,category='operational-readiness-certification')
    def open_operations_command_center(self):
        project=self.project_controller.current_project
        dialog=OperationsCommandCenterDialog(
            self.operations_command_center_service,
            self,
            project_id=project.project_id if project else None,
            open_path=self.open_path,
    ...
    def open_provider_governance(self):
        ...
        dialog=ProviderGovernanceDialog(
            self.provider_governance_service,
            self,
            project_id=project.project_id if project else None,
            open_path=self.open_path,
        )
        return self._show_report_dialog(dialog,category='provider-governance')
